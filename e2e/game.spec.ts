...
async function openBuildDialog(page: Page): Promise<void> {
  await page.locator('#hud-open-build-dialog').click();
  await expect(page.getByTestId('dialog-title')).toContainText('Build');
}

async function openAttractionsDialog(page: Page): Promise<void> {
  await page.locator('#hud-open-attractions-dialog').click();
  await expect(page.getByTestId('dialog-title')).toContainText('Attractions');
}

async function selectBuildTool(page: Page, tabTestId: string, toolTestId: string): Promise<void> {
  await openBuildDialog(page);
  await page.getByTestId(tabTestId).click();
  await page.getByTestId(toolTestId).click();
}

async function selectAttractionTab(page: Page, tabTestId: string): Promise<void> {
  await openAttractionsDialog(page);
  await page.getByTestId(tabTestId).click();
}
...
test('can place multiple attractions of same type', async ({ page }) => {
  await selectAttractionTab(page, 'tab-food');
...
test('selecting attraction opens details dialog and saves custom values', async ({ page }) => {
  ...
  await selectBuildTool(page, 'build-tab-tools', 'tool-select');
  await clickTile(page, 21, 22);
...
test('attraction build dialog rotates orientation via one button', async ({ page }) => {
  await selectAttractionTab(page, 'tab-fun');
...
test('non-stall attractions require side-adjacent entrance and exit tiles', async ({ page }) => {
  await selectBuildTool(page, 'build-tab-path', 'tool-path');
