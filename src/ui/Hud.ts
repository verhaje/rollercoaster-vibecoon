...
export class HUD {
  ...
  private attractionPanel: HTMLDivElement | null = null;
  private tabBar: HTMLDivElement | null = null;
  private buildTabBar: HTMLDivElement | null = null;
  ...
  private openAttractionsDialogBtn: HTMLButtonElement | null = null;
  ...
  private showBuildToolsDialog(): void {
    ...
    const box = this.openDialog('🛠 Build', `
      <div class="hud-build-tabs" id="hud-dialog-build-tabs">
        <button class="build-tab-btn active" data-build-tab="land" data-testid="build-tab-land" type="button">Land</button>
        <button class="build-tab-btn" data-build-tab="path" data-testid="build-tab-path" type="button">Path</button>
        <button class="build-tab-btn" data-build-tab="trees" data-testid="build-tab-trees" type="button">Trees</button>
        <button class="build-tab-btn" data-build-tab="scenery" data-testid="build-tab-scenery" type="button">Scenery</button>
        <button class="build-tab-btn" data-build-tab="tools" data-testid="build-tab-tools" type="button">Tools</button>
      </div>
      <div class="hud-build-groups" id="hud-dialog-build-groups" style="margin-top:8px;">
        <div class="build-group" data-build-group="land">
          <button class="tool-btn hud-dialog-tool-btn" data-tool="land" data-variant="0" data-testid="tool-land-grass" type="button">🟩 Grass</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="land" data-variant="1" data-testid="tool-land-desert" type="button">🟨 Desert</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="land" data-variant="2" data-testid="tool-land-mud" type="button">🟫 Mud</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="land" data-variant="3" data-testid="tool-land-dark-grass" type="button">🟩 Dark Grass</button>
        </div>
        <div class="build-group" data-build-group="path" hidden>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="path" data-variant="0" data-testid="tool-path" type="button">🟫 Mud Path</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="path" data-variant="1" data-testid="tool-path-desert" type="button">🟨 Desert Path</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="path" data-variant="2" data-testid="tool-path-concrete" type="button">⬜ Concrete Path</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="path" data-variant="3" data-testid="tool-path-queue" type="button">🟪 Queue Path</button>
        </div>
        ...
        <div class="build-group" data-build-group="tools" hidden>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="terrain" data-testid="tool-terrain" type="button">⛰ Terrain</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="water" data-testid="tool-water" type="button">💧 Water</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="pickup" data-testid="tool-pickup" type="button">✋ Pickup</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="bench" data-testid="tool-bench" type="button">🪑 Bench</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="trashcan" data-testid="tool-trashcan" type="button">🗑 Trash Can</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="demolish" data-testid="tool-demolish" type="button">🔨 Demolish</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="select" data-testid="tool-select" type="button">🔍 Select</button>
          <button class="tool-btn hud-dialog-tool-btn" data-tool="none" data-testid="tool-none" type="button">❌ Cancel</button>
        </div>
      </div>
    `);
  ...
  private updateMobileLaunchersVisibility(): void {
    ...
    this.mobileLaunchersEl.hidden = false;
  }
  ...
  private build(): void {
    ...
    this.openAttractionsDialogBtn = this.root.querySelector('#hud-open-attractions-dialog');
    ...
    this.openAttractionsDialogBtn?.addEventListener('click', () => {
      this.showAttractionsDialog();
    });
  ...
  private renderAttractionButtons(container: HTMLElement): void {
    container.innerHTML = '';
  ...
  private showAttractionsDialog(): void {
    const tabs: Array<{ key: AttractionTab; label: string; icon: string }> = [
    ...
    const tabButtons = tabs
      .map((tab) => {
        const activeClass = tab.key === this.activeTab ? ' active' : '';
        return `<button class="tab-btn${activeClass}" data-tab="${tab.key}" data-testid="tab-${tab.key}" type="button">${tab.icon} ${tab.label}</button>`;
      })
      .join('');

    const box = this.openDialog('🎢 Attractions', `
      <div class="hud-tabs" id="dlg-attraction-tabs">${tabButtons}</div>
      <div class="hud-attractions" id="dlg-attractions"></div>
    `);

    const attractionContainer = box.querySelector('#dlg-attractions') as HTMLDivElement;
    this.renderAttractionButtons(attractionContainer);

    box.querySelectorAll<HTMLButtonElement>('#dlg-attraction-tabs .tab-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        const nextTab = btn.dataset.tab as AttractionTab | undefined;
        if (!nextTab) return;
        this.activeTab = nextTab;
        box.querySelectorAll('#dlg-attraction-tabs .tab-btn').forEach((other) => other.classList.remove('active'));
  ...
  refresh(): void {
    ...
    const attrBtns = this.root.querySelectorAll('.attr-btn');
