...
import {
  advanceCartProgress,
  cartTextureKey,
  normalizeCornerSpriteOrientation,
  resolveCornerRenderType,
  trackTextureKey,
} from './utils/coasterRenderState';
    ...
    if (this.coasterTracks) {
      ...
      if (build) {
        ...
        for (let pi = 0; pi < build.pieces.length; pi++) {
          ...
          const spriteCorner = normalizeCornerSpriteOrientation(renderPieceType, piece.direction);
          ...
          for (let ti = 0; ti < pieceTiles.length; ti++) {
            ...
            const texKey = `${trackTextureKey(spriteCorner.pieceType, spriteCorner.direction, build.trackColor, piece.height, buildStyle)}_${hillTransition}_${isSecondaryTile ? 1 : 0}`;
            const drawer = getTrackPieceDrawer(
              spriteCorner.pieceType,
              spriteCorner.direction,
              build.trackColor,
              piece.height,
              isSecondaryTile,
              hillTransition,
      ...
      for (const [instId, track] of this.coasterTracks.tracks) {
        ...
        for (let pi = 0; pi < track.pieces.length; pi++) {
          ...
          for (let ti = 0; ti < pieceTiles.length; ti++) {
            ...
            const surfaceH = Math.max(sim.getTileHeight(tile.x, tile.y), sim.getUpperPathHeight(tile.x, tile.y));
            spr.y = tile.y * T - (surfaceH + piece.height) * TERRAIN_PIXEL_STEP;
            const texKey = `${trackTextureKey(spriteCorner.pieceType, spriteCorner.direction, track.trackColor, piece.height, trackStyle)}_${hillTransition}_${isSecondaryTile ? 1 : 0}`;
            const drawer = getTrackPieceDrawer(
              spriteCorner.pieceType,
              spriteCorner.direction,
              track.trackColor,
