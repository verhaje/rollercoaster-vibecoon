...
export function normalizeCornerSpriteOrientation(
  pieceType: number,
  direction: number,
): { pieceType: number; direction: number } {
  // Use one canonical small-corner sprite and rotate by direction.
  // In this codebase, TurnLeft is visually equivalent to TurnRight rotated +90°.
  if (pieceType === 1) {
    return {
      pieceType: 2,
