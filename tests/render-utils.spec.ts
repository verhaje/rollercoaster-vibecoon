...
describe('coaster render state utils', () => {
  ...
  it('normalizes small-turn sprites to one canonical corner and rotates direction', () => {
    expect(normalizeCornerSpriteOrientation(1, 0)).toEqual({ pieceType: 2, direction: 1 });
    expect(normalizeCornerSpriteOrientation(1, 3)).toEqual({ pieceType: 2, direction: 0 });
    expect(normalizeCornerSpriteOrientation(2, 2)).toEqual({ pieceType: 2, direction: 2 });
    expect(normalizeCornerSpriteOrientation(0, 1)).toEqual({ pieceType: 0, direction: 1 });
  });
});
